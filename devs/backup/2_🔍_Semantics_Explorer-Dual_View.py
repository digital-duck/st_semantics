...
class EnhancedDualViewManager:
    ...
    def create_enhanced_dual_view(self, embeddings, labels, colors, title, zoom_params):
        """Create enhanced dual view with center/size based zoom"""
        ...
        fig = make_subplots(
            rows=2, cols=1,
            subplot_titles=('📊 Overview', '🔍 Detail View'),
            row_heights=[0.4, 0.6],
        ...
        # OVERVIEW PANEL - All points, small markers
        if np.any(chinese_mask):
            fig.add_trace(go.Scatter(
                ...
                mode='markers',
                marker=dict(
                    size=self.overview_config['marker_size'],
                    color='crimson',
                    opacity=self.overview_config['opacity'],
        ...
        # DETAIL PANEL - Only points within viewport
        viewport_mask = (
        ...
        if np.any(chinese_viewport):
            fig.add_trace(go.Scatter(
                ...
                mode='markers+text',
                marker=dict(
                    size=self.detail_config['marker_size'],
                    color='crimson',
                    opacity=self.detail_config['opacity'],
                    line=dict(width=2, color='white')
                ),
                text=[labels[i] for i in range(len(labels)) if chinese_viewport[i]],
                textposition="top center",
                textfont=dict(
                    size=self.detail_config['textfont_size'], 
                    color='darkred', 
                    family='Arial Black'
                ),
        ...
        fig.update_layout(
            title=dict(
                text=f"🔍 Enhanced Dual View: {title}",
                x=0.5,
                font=dict(size=18)
            ),
            dragmode='pan',
        ...
        return fig, points_in_viewport, viewport_mask
...
def main():
        ...
        with st.expander("Zoom Controls", expanded=True):
            ...
            if 'zoom_params' not in st.session_state:
                st.session_state.zoom_params = {
                    ...
                    'width': global_width * 0.25, 'height': global_height * 0.25,  # 25% of global
                    'delta_x': global_width * 0.05, 'delta_y': global_height * 0.05  # 5% of global
                }
            ...
            col1, col2 = st.columns(2)
            with col1:
                center_x = st.number_input("Center X", value=zoom_params['center_x'], step=0.001, format="%.3f")
            with col2:
                center_y = st.number_input("Center Y", value=zoom_params['center_y'], step=0.001, format="%.3f")
            
            st.write("**Box Size:**")
            col3, col4 = st.columns(2)
            with col3:
                width = st.number_input("Width", value=zoom_params['width'], step=0.001, format="%.3f", min_value=0.001)
            with col4:
                height = st.number_input("Height", value=zoom_params['height'], step=0.001, format="%.3f", min_value=0.001)
            
            st.write("**Panning Step:**")
            col5, col6 = st.columns(2)
            with col5:
                delta_x = st.number_input("Delta X", value=zoom_params['delta_x'], step=0.001, format="%.3f")
            with col6:
                delta_y = st.number_input("Delta Y", value=zoom_params['delta_y'], step=0.001, format="%.3f")
    ...
    if 'enhanced_data' in st.session_state:
        ...
        fig, points_count, viewport_mask = dual_manager.create_enhanced_dual_view(
            data['embeddings'],
            data['labels'],
            data['colors'],
            data['title'],
        ...
        # Create two separate figures for overview and detail
        overview_fig = go.Figure()
        detail_fig = go.Figure()
        
        # Separate data by language for overview
        chinese_mask = np.array([color == 'chinese' for color in data['colors']])
        english_mask = np.array([color == 'english' for color in data['colors']])
        
        # OVERVIEW FIGURE - All points, small markers
        if np.any(chinese_mask):
            overview_fig.add_trace(go.Scatter(
                x=data['embeddings'][chinese_mask, 0],
                y=data['embeddings'][chinese_mask, 1],
                mode='markers',
                marker=dict(size=6, color='crimson', opacity=0.8, line=dict(width=1, color='white')),
                text=[data['labels'][i] for i in range(len(data['labels'])) if chinese_mask[i]],
                hovertemplate='<b>%{text}</b><br>中文<br>x: %{x:.3f}<br>y: %{y:.3f}<extra></extra>',
                name="中文",
                showlegend=True
            ))
        ...
        if np.any(english_mask):
            overview_fig.add_trace(go.Scatter(
                x=data['embeddings'][english_mask, 0],
                y=data['embeddings'][english_mask, 1],
                mode='markers',
                marker=dict(size=6, color='steelblue', opacity=0.8, line=dict(width=1, color='white')),
                text=[data['labels'][i] for i in range(len(data['labels'])) if english_mask[i]],
                hovertemplate='<b>%{text}</b><br>English<br>x: %{x:.3f}<br>y: %{y:.3f}<extra></extra>',
                name="English",
                showlegend=True
            ))
        ...
        # Add zoom box to overview
        viewport_coords = dual_manager.center_size_to_bounds(
            st.session_state.zoom_params['center_x'], st.session_state.zoom_params['center_y'],
            st.session_state.zoom_params['width'], st.session_state.zoom_params['height']
        )
        ...
        data_x_min, data_x_max = data['embeddings'][:, 0].min(), data['embeddings'][:, 0].max()
        data_y_min, data_y_max = data['embeddings'][:, 1].min(), data['embeddings'][:, 1].max()
        x_padding = (data_x_max - data_x_min) * 0.1
        y_padding = (data_y_max - data_y_min) * 0.1
        ...
        # DETAIL FIGURE - Only points within viewport
        viewport_mask = (
            (data['embeddings'][:, 0] >= viewport_coords['x_min']) &
            (data['embeddings'][:, 0] <= viewport_coords['x_max']) &
            (data['embeddings'][:, 1] >= viewport_coords['y_min']) &
            (data['embeddings'][:, 1] <= viewport_coords['y_max'])
        )
        ...
        if np.any(chinese_viewport):
            detail_fig.add_trace(go.Scatter(
                x=data['embeddings'][chinese_viewport, 0],
                y=data['embeddings'][chinese_viewport, 1],
                mode='markers+text',
                marker=dict(size=16, color='crimson', opacity=1.0, line=dict(width=2, color='white')),
                text=[data['labels'][i] for i in range(len(data['labels'])) if chinese_viewport[i]],
                textposition="top center",
        ...
        if np.any(english_viewport):
            detail_fig.add_trace(go.Scatter(
                x=data['embeddings'][english_viewport, 0],
                y=data['embeddings'][english_viewport, 1],
                mode='markers+text',
                marker=dict(size=16, color='steelblue', opacity=1.0, line=dict(width=2, color='white')),
                text=[data['labels'][i] for i in range(len(data['labels'])) if english_viewport[i]],
                textposition="top center",
        ...
        _, btn_pan_col, _ = st.columns([5, 5, 2])
        with btn_pan_col:
            btn_pan = st.button("Panning", type="primary", help="Apply pan movement from zoom controls")
        if btn_pan:
